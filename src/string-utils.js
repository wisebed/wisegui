...
StringUtils.replacements = replacements;
 ...
 **/
StringUtils.string2int = function(input) {
	var radix = 10;
...
StringUtils.makePrintable = function(arr) {
	var res = '';
...
StringUtils.padl = function(str, pad, num) {
	return (Array(num+1).join(pad) + str).substring(str.length);
};

StringUtils.toXArray = function(str, radix) {
	var res = [];
...
StringUtils.toHexArray = function(str) {
		return toXArray(str, 16);
	};

StringUtils.toDecimalArray = function(str) {
	return toXArray(str, 10);
};

StringUtils.toBinaryArray = function(str) {
	return toXArray(str, 2);
};

StringUtils.toHexString = function(str) {
	return toXstring(str, 16).join(' ');
};

StringUtils.toDecimalString = function(str) {
	return toXstring(str, 10).join(' ');
};

StringUtils.toBinaryString = function(str) {
	return toXstring(str, 2).join(' ');
};

StringUtils.startsWith =  function (str, patt) {
